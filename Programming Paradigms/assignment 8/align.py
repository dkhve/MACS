...
def findOptimalAlignment(strand1, strand2, aligmentResults):

	# if one of the two strands is empty, then there is only
	# one possible alignment, and of course it's optimal
	if (strand1, strand2) in aligmentResults:
		return aligmentResults[(strand1, strand2)]

	if len(strand1) == 0:
		aligmentResults[strand1, strand2] = len(strand2) * -2, \
								 (' ' * len(strand2), ' ' * len(strand2), strand2, '2' * len(strand2))
		return aligmentResults[strand1,strand2]

	if len(strand2) == 0:
		aligmentResults[strand1, strand2] = len(strand1) * -2, \
								 (' ' * len(strand1), strand1, ' ' * len(strand1), '2' * len(strand1))
		return aligmentResults[strand1, strand2]

	# There's the scenario where the two leading bases of
	# each strand are forced to align, regardless of whether or not
	# they actually match.
	bestWith, sequence = findOptimalAlignment(strand1[1:], strand2[1:], aligmentResults)
	if strand1[0] == strand2[0]:
		aligmentResults[strand1, strand2] = bestWith + 1, ('1' + sequence[0], strand1[0] + sequence[1],
							  strand2[0] + sequence[2], ' ' + sequence[3])
		return  aligmentResults[strand1, strand2]
	# no benefit from making other recursive calls

	best = bestWith - 1
	sequence = (' ' + sequence[0], strand1[0] + sequence[1], strand2[0] + sequence[2], '1' + sequence[3])

	# It's possible that the leading base of strand1 best
	# matches not the leading base of strand2, but the one after it.
	bestWithout, seq = findOptimalAlignment(strand1, strand2[1:], aligmentResults)

	bestWithout -= 2 # penalize for insertion of space
	seq = (' ' + seq[0], ' ' + seq[1], strand2[0] + seq[2], '2' + seq[3])
	...
	# opposite scenario
	bestWithout, seq = findOptimalAlignment(strand1[1:], strand2, aligmentResults)
	bestWithout -= 2  # penalize for insertion of space
	seq = (' ' + seq[0], strand1[0] + seq[1], ' ' + seq[2], '2' + seq[3])

	if bestWithout > best:
		best = bestWithout
		sequence = seq

	aligmentResults[strand1, strand2] = best, sequence
	return aligmentResults[strand1, strand2]
...
def main():
	while (True):
		...
		if answer == "no\n": break
		strand1 = generateRandomDNAStrand(800, 995)
		strand2 = generateRandomDNAStrand(800, 995)
		sys.stdout.write("Aligning these two strands: " + strand1 + "\n")
		sys.stdout.write("                            " + strand2 + "\n")
		aligmentResults = {}
		alignment, seq = findOptimalAlignment(strand1, strand2, aligmentResults)
		printAlignment(alignment)
		test(alignment, seq[0], seq[3], seq[1], seq[2])
