...
(define (listPoints circle-list)
     (if (null? circle-list) '()
        (append (map (lambda (elem)
                        (intersect (car circle-list) elem))
                    (cdr circle-list))
                (listPoints (cdr circle-list)))))


(define (intersection-points circle-list)
    (apply append (listPoints circle-list)))

(define (computeDistances point point-list)
    (map (lambda (elem)
            (dist point elem))
          (remove point point-list)))

(define (distance-product point point-list)
    (apply * (computeDistances point point-list)))   

(define (rate-points point-list) 
      (map (lambda (elem)
              (list (distance-product elem point-list) elem))
          point-list))
...
(define (clumped-points point-list)
  (let ((sorted-by-rate (sort-points (rate-points point-list))))
    (apply append
      (map cdr 
          (prefix-of-list sorted-by-rate
                          (div (length sorted-by-rate) 2))))))

(define (average-point point-list)
  (let* ((len (length point-list))
        (averagePoint (make-pt (/ (apply + (map x point-list)) len)
                               (/ (apply + (map y point-list)) len))))
    (list (distance-product averagePoint point-list) averagePoint)))
...
(define (where-am-i distances starLocs)
    (sort-points (map best-estimate (all-guesses distances starLocs))))
