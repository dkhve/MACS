 ...
 */
int Report_Transfer(Bank *bank, int workerNum, AccountNumber accountNum, AccountAmount amount){
  sem_wait(&bank->transferPadlock); //the report transfer function must happen atomically
  ...
  if (amountAbs < reportingAmount) {
     sem_post(&bank->transferPadlock); // let other threads make report about transfer
...
//wakes up other paused threads
void wakeUpOthers(Bank* bank, int workerNum){
  for(int j = 0 ; j < numWorkers; j++){
    if(j != workerNum) {
      sem_t* ptr = bank->workersFinished + j;
      sem_post(ptr);
    }
  }
}

//does all neccessary actions to end a day
void endDay(Bank* bank, int workerNum){
  wakeUpOthers(bank, workerNum); //wakes up "sleeping" workers
  bank->activeWorkers = numWorkers; //every worker has to start their job anew, so they become active for next day
  sem_post(&bank->checkHelper); // so that next day's report can be done
}
...
int Report_DoReport(Bank *bank, int workerNum){
  sem_wait(&bank->checkHelper);// so that checking if this worker is last one who should do the report happens atomically
  bank->activeWorkers--; //there is one less active worker
  if(bank->activeWorkers){
    sem_post(&bank->checkHelper); // so that other threads can check too
    sem_wait(&bank->workersFinished[workerNum]); //reporting that this worker is finished and waiting for next day
    return 0;
  }
  //if upper if doesn't happen, it means last worker has come and thats what we need
  //we need last worker to make daily report, that means every other is finished and only one worker makes report 
  //also no other worker should start working before report/day is finished 
  Report *rpt = bank->report;
  assert(rpt); Y;
  if (rpt->numReports >= MAX_NUM_REPORTS) {
      // We've run out of report storage for the bank
      endDay(bank, workerNum);
      return -1;
  }
  /*
...
int Report_Compare(Bank *bank1, Bank *bank2){
  ...
  for (int r = 0; r < rpt1->numReports; r++) {
    ...
    if (!rpt1->dailyData[r].hasOverflowed) {
      ...
      qsort(rpt1->dailyData[r].transferLog, n, sizeof(struct TransferLog), TransferLogSortFunc);
      assert(n == rpt2->dailyData[r].numLogEntries);
      qsort(rpt2->dailyData[r].transferLog, n, sizeof(struct TransferLog), TransferLogSortFunc);
      for (i = 0; i < n; i++) {
