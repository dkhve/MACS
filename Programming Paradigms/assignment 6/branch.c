...
//initializes single branch
void initSingleBranch(Branch* branch, int ID, int accountNum){
  sem_init(&branch->BranchLocker, 0, 1); 
  branch->branchID = ID;
  branch->balance = 0;
  branch->numberAccounts = accountNum;
  branch->accounts = (Account*)malloc(accountNum * sizeof(Account));
}
...
int Branch_Init(Bank *bank, int numBranches, int numAccounts, AccountAmount initialAmount){
  ...
  for (int i = 0; i < numBranches; i++) {
    ...
    initSingleBranch(branch, i, accountsPerBranch);
...
int Branch_UpdateBalance(Bank *bank, BranchID branchID, AccountAmount change){
  ...
  if (branchID >= bank->numberBranches) return -1;
...
int Branch_Compare(Branch *branch1, Branch *branch2){
  ...
  for (int i = 0; i < branch1->numberAccounts; i++) {
    assert(branch1->accounts[i].accountNumber == branch2->accounts[i].accountNumber);
