...
void initSemaphores(Bank* bank, int numWorkers){
  sem_init(&bank->transferPadlock, 0, 1);
  sem_init(&bank->checkHelper, 0, 1);
  bank->workersFinished = malloc(numWorkers*sizeof(sem_t));
 ...
 */
Bank* Bank_Init(int numBranches, int numAccounts, AccountAmount initalAmount, AccountAmount reportingAmount, int numWorkers){
  Bank *bank = malloc(sizeof(Bank));
  bank->activeWorkers = numWorkers;
  initSemaphores(bank, numWorkers);
...
int Bank_Balance(Bank *bank, AccountAmount *balance){
  ...
  for(int j = 0; j < bank->numberBranches; j++){
    sem_wait(&bank->branches[j].BranchLocker);
  }
  ...
  *balance = bankTotal;
  for(int j = 0; j < bank->numberBranches; j++){
    sem_post(&bank->branches[j].BranchLocker);
  }
  return 0;
}
...
int Bank_Compare(Bank *bank1, Bank *bank2){
  ...
  for (unsigned int branch = 0; branch < bank1->numberBranches; branch++) {
    int berr = Branch_Compare(&bank1->branches[branch], &bank2->branches[branch]);
    if (berr < 0) err = berr;
  }
