...
void Account_Init(Bank *bank, Account *account, int id, int branch, AccountAmount initialAmount){
  ...
  //initializing the semaphore which "protects" this account from being changed by more than one thread simultaneously
  sem_init(&account->padlock, 0, 1);
...
void Account_Adjust(Bank *bank, Account *account, AccountAmount amount, int updateBranch){
  ...
  if (updateBranch) Branch_UpdateBalance(bank, AccountNum_GetBranchID(account->accountNumber), amount);
  Y;
}
...
int Account_IsSameBranch(AccountNumber accountNum1, AccountNumber accountNum2){
  return (AccountNum_GetBranchID(accountNum1) == AccountNum_GetBranchID(accountNum2));
}
