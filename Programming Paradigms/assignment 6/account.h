...
typedef struct Account {
  ...
  sem_t padlock; //added lock, so that no two thread can access same account an the same time and change its content
} Account;
...
void Account_Adjust(struct Bank *bank, Account *account, AccountAmount amount, int updateBranch);
