...
//generates random text with given grammar
void generateText(vector<string> &result,  map<string, Definition> &grammar, Definition &def){
  if(def.isEmpty()) return; //check if definition contains any productions at all

  Production product = def.getRandomProduction();

  for(vector<string>::iterator it = product.begin(); it != product.end(); ++it){
    string curr = *it;

    if(grammar.find(curr) != grammar.end()){  //we've come across a non-terminal
      Definition d = grammar[curr];
      generateText(result, grammar, d);
    }

    else if(curr[0] != '<' || curr[curr.size()-1] != '>'){  //check if we want to add this string to the text
      result.push_back(curr);
    }
    
    else {   //we've come across a non-terminal that is not in the map
      cerr<<"Could not find "<< curr <<" in the grammar file."<<endl;
      exit(0);
    }
...
//prints the generated text that is contained within result vector
void printText(vector<string> &result){
...
int main(int argc, char *argv[])
{
  ...
  for(int i = 1 ; i <= 3; i++){
    ...
    generateText(result, grammar, startDef);
    printf("Version #%d: --------------------------", i);
    printf("\n");
    printText(result);
    printf("\n");
