...
public class Piece {
	...
	/**
	 ...
	 and sorts the points array by x coordinates first and y coordinates second
	*/
	...
	// sorts array by x coordinates, if same then by y coordinates.
	...
	private void buildSkirt() {
		...
		for(int currX = 0; currX < width; currX++){
			...
			while(index < body.length && body[index].x == currX){
				minY = Math.min(body[index].y, minY);
				index++;
			}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		...
		Piece nextRot;
		while (true){
			nextRot = thisRot.computeNextRotation();
			if(nextRot.equals(root)) {
				thisRot.next = root;
				break;
			}
			thisRot.next = nextRot;
			thisRot = nextRot;
		}
		return root;
	}
