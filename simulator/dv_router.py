...
class DVRouter(basics.DVRouterBase):
    ...
    def handle_link_down(self, port):
        ...
        for host in self.table:  # for host in hosts
            if port == self.table[host][1]:  # if I could reach that entity from this port
                to_be_removed.append(host)  # remove that route from my table
                if self.POISON_MODE:
    ...
    def handle_rx(self, packet, port):
        ...
        if isinstance(packet, basics.RoutePacket):
            ...
            if dest in self.table and new_cost >= INFINITY and new_cost > self.table[dest][2]:
                update_packet = basics.RoutePacket(dest, self.table[dest][2])
                self.send(update_packet, flood=True)
            elif dest not in self.table or new_cost <= self.table[dest][2] or self.table[dest][1] == port:
                if new_cost < INFINITY:
                    self.table[dest] = (packet.dst, port, new_cost, api.current_time())
                update_packet = basics.RoutePacket(dest, new_cost)
    ...
    def handle_timer(self):
        ...
        """
        self.remove_expired()
        print self.name + ": " + str(self.table)
        ...
        for host in self.table:
            ...
            info = basics.RoutePacket(host, self.table[host][2])  # 2 is index of latency
            self.send(info, route_port, flood=True)
