...
class ExternalTaskSensor(BaseSensorOperator):
    ...
    def __init__(self,
        ...
        self.time = datetime.datetime.now()     # 1
        self.time_delta = datetime.timedelta(seconds=120)  # 2
    ...
    def poke(self, context, session=None):
        ...
        self.time = self.time.replace(tzinfo=dttm_filter[0].tzinfo)  # 3
        for dag_id in self.external_dag_id:  # 4
            if self.external_task_id:
                count = session.query(TI).filter(
                    TI.dag_id == dag_id,  # 5
                    TI.task_id == self.external_task_id,
                    TI.state.in_(self.allowed_states),
                    TI.execution_date.between(self.time, self.time+self.time_delta), # 6
                ).count()
            else:
                count = session.query(DR).filter(
                    DR.dag_id == dag_id,  # 7
                    DR.state.in_(self.allowed_states),
                    DR.execution_date.between(self.time, self.time+self.time_delta),  # 8
                ).count()

            session.commit()
            if count != len(dttm_filter):  # 9
                return False    # 10
