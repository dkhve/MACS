...
void initPQ(PriorityQueue<Node*> &pq, Map<ext_char, int>& frequencies);
void uniteFirstTwo(PriorityQueue<Node*> &pq);
void initCode(Node* encodingTree, Map<ext_char, Queue<bool> > &code, Queue<bool> &q);
...
Node* buildEncodingTree(Map<ext_char, int>& frequencies) {
	...
	initPQ(pq, frequencies);
	while(pq.size() > 1) uniteFirstTwo(pq);
	return pq.dequeue();
}
...
void initCode(Node* cell, Map<ext_char, Queue<bool> > &code, Queue<bool> &q){
	if(cell == NULL) return;
	...
	Queue<bool> tmp = q;
	tmp.enqueue(false);
	initCode(cell->zero, code, tmp);
	tmp = q;
	tmp.enqueue(true);
	initCode(cell->one, code, tmp);
}
